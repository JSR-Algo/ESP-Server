...
validate_production_boot_env() {
  ...
  local lesson_runtime lesson_sample lesson_renderer_v2 lesson_renderer_v3 lesson_renderer_v4 lesson_renderer_v5 lesson_motion lesson_playful rollout_allowlist rollout_count
  ...
  lesson_renderer_v3="$(env_value LESSON_RENDERER_V3_ENABLED "false")"
  lesson_renderer_v5="$(env_value LESSON_RENDERER_V5_ENABLED "false")"
  ...
  for key in LESSON_RUNTIME_ENABLED LESSON_SAMPLE_ENABLED LESSON_RENDERER_V2_ENABLED LESSON_RENDERER_V3_ENABLED LESSON_RENDERER_V4_ENABLED LESSON_RENDERER_V5_ENABLED LESSON_MOTION_PRESETS_ENABLED LESSON_PLAYFUL_INTERACTIONS_ENABLED; do
  ...
  if [[ "${lesson_runtime}" == "false" && "${lesson_renderer_v3}" == "true" ]]; then
    printf 'error: LESSON_RENDERER_V3_ENABLED requires LESSON_RUNTIME_ENABLED=true\n' >&2
    failed=1
  fi
  ...
  if [[ "${lesson_runtime}" == "false" && "${lesson_renderer_v5}" == "true" ]]; then
    printf 'error: renderer v5 cannot be true while LESSON_RUNTIME_ENABLED is false\n' >&2
    failed=1
  fi
