...
@Tag(name = "Device management", description = "OTA related APIs")
@Slf4j
@RestController
...
public class OTAController {
    ...
    public ResponseEntity<String> checkOTAVersion(
        ...
        // DeviceIdandMacAddress should be consistent, and must needapplicationField
        if (!macAddressValid) {
    ...
    @Operation(summary = "Quick check device activation status")
    ...
    public ResponseEntity<String> getOTA() {
        ...
        if (StringUtils.isBlank(mqttUdpConfig)) {
            return ResponseEntity.ok("OTA API not working, missing mqtt_gateway address. Log in to Smart Control Console, find [server.mqtt_gateway] config in parameter management");
        }
        String wsUrl = sysParamsService.getValue(Constant.SERVER_WEBSOCKET, true);
        if (StringUtils.isBlank(wsUrl) || wsUrl.equals("null")) {
            return ResponseEntity.ok("OTA API abnormal, missing websocket address. Log in to Smart Control Panel, find [server.websocket] config in Parameter Management");
        }
        String otaUrl = sysParamsService.getValue(Constant.SERVER_OTA, true);
        if (StringUtils.isBlank(otaUrl) || otaUrl.equals("null")) {
            return ResponseEntity.ok("OTA API abnormal, missing ota address. Log in to Smart Control Panel, find [server.ota] config in Parameter Management");
        }
        return ResponseEntity.ok("OTA API running normally, websocket cluster count:" + wsUrl.split(";").length);
    }
    ...
    /**
     * Simple Checkmac addressWhether valid (non-strict)
     * 
     * @param macAddress
     * @return
     */
    private boolean isMacAddressValid(String macAddress) {
        ...
        // MAC addressUsually is12hex digits, may contain colons or hyphensSeparator
