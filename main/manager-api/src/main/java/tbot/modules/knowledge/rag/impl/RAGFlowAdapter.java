package tbot.modules.knowledge.rag.impl;
...
import tbot.modules.knowledge.rag.RAGFlowClient;

/**
 * RAGFlowKnowledge base adapter implementation
 * <p>
 * Refactor Notes (Refactoring Note):
 * This class upgraded to use {@link RAGFlowClient} Unified Processing HTTP Communication.
 * Fixed old code in Timeout Missing,Error Handling scattered questions.
 * </p>
 */
@Slf4j
public class RAGFlowAdapter extends KnowledgeBaseAdapter {
    ...
    // Client Instance, created on initialization
    ...
    public void initialize(Map<String, Object> config) {
        ...
        // Initialize ClientDefault timeout 30s, can through config Extend
        int timeout = 30;
        ...
        if (timeoutObj != null) {
            try {
                ...
                log.warn("Failed to parse timeout config, using default 30s");
            }
        }
        this.client = new RAGFlowClient(baseUrl, apiKey, timeout);
        log.info("RAGFlow adapter initialization complete, Client ready");
    }
    ...
    /**
     * Helper method: support getting config by multiple key names (compatible camelCase and snake_case)
     */
    ...
    /**
     * Helper method: ensure Client Initialized
     */
    ...
    public PageData<KnowledgeFilesDTO> getDocumentList(String datasetId, DocumentDTO.ListReq req) {
        try {
            ...
            // Use Jackson will DTO Convert to Map As query parameter
            ...
            log.error("Get document list failed", e);
    ...
    public KnowledgeFilesDTO uploadDocument(DocumentDTO.UploadReq req) {
        ...
        try {
            ...
            if (req.getChunkMethod() != null) {
                // Convert enum value to RAGFlow Expected string (such as NAIVE -> naive)
    ...
    public boolean parseDocuments(String datasetId, List<String> documentIds) {
        try {
            log.info("=== [RAGFlow] Parse document ===");
    ...
    public ChunkDTO.ListVO listChunks(String datasetId, String documentId, ChunkDTO.ListReq req) {
        try {
            // [Lantern Refactor] Use objectMapper Dynamically convert query parameters, eliminate hardcoding
            ...
            Object dataObj = response.get("data");
            if (dataObj == null) {
                log.warn("[RAGFlow] listChunks Response data Empty, docId={}", documentId);
            ...
            log.error("Get chunks failed: docId={}", documentId, e);
            throw convertToRenException(e);
        }
    }
    ...
    public RetrievalDTO.ResultVO retrievalTest(RetrievalDTO.TestReq req) {
        try {
            // [Production Reinforce] Parameter defensive alignment:RAGFlow Python End to 0 Or negativePaginationSensitive
            // Solve ValueError('Search does not support negative slicing.')
            if (req.getPage() != null && req.getPage() < 1) {
                req.setPage(1);
            }
            if (req.getPageSize() != null && req.getPageSize() < 1) {
                req.setPageSize(10); // Default 10 item
            }
            ...
            // [Lantern Refactor] Direct pass-through strong type DTO, by getClient Handle serialization
            Map<String, Object> response = getClient().post("/api/v1/retrieval", req);

            Object dataObj = response.get("data");
            if (dataObj == null) {
                log.warn("[RAGFlow] retrievalTest response data empty");
    ...
    public DatasetDTO.InfoVO createDataset(DatasetDTO.CreateReq req) {
        try {
            // [Production Fix] Strengthen default value handling, prevent RAGFlow API Error due to empty string or missing field (Code 101)
            // Solve "Field: <avatar> - Message: <Missing MIME prefix>" And validation failure
            ...
            // 🤖 Auto-complete embedding model: prefer request parameters, then default model in config
            if (StringUtils.isBlank(req.getEmbeddingModel())) {
                ...
                if (StringUtils.isNotBlank(defaultModel)) {
                    log.info("RAGFlow: using default embedding model from config: {}", defaultModel);
                    req.setEmbeddingModel(defaultModel);
                }
                // If config also has no default, leave empty for RAGFlow Server side fallback itself (or throw businessException)
            }

            // 🖼️ Auto CompleteAvatarIf empty provide one 1x1 Transparent pixels, prevent RAGFlow Validate MIME Prefix Fail
            if (StringUtils.isBlank(req.getAvatar())) {
                req.setAvatar(
                        "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmMIQAAAABJRU5ErkJggg==");
            }

            // Pass strongly typed request object directly to Client,Jackson Will handle JsonProperty Map
            Map<String, Object> response = getClient().post("/api/v1/datasets", req);

            // Safely get data And pass DatasetDTO.InfoVO Do full mapping
            Object dataObj = response.get("data");
            if (dataObj != null) {
    ...
    public DatasetDTO.InfoVO updateDataset(String datasetId, DatasetDTO.UpdateReq req) {
        try {
            // RAGFlow API Update suggested path with ID
            ...
            log.error("Update dataset failed", e);
            throw convertToRenException(e);
        }
    }
    ...
    public DatasetDTO.BatchOperationVO deleteDataset(DatasetDTO.BatchIdReq req) {
        try {
            // RAGFlow BatchDeleteAPI Usage DELETE /api/v1/datasets
            ...
            log.error("Batch delete datasets failed", e);
            throw convertToRenException(e);
        }
    }
    ...
    public DatasetDTO.InfoVO getDatasetInfo(String datasetId) {
        try {
            ...
            // RAGFlow Dataset not exist on server
            return null;
        } catch (Exception e) {
            log.warn("Failed to get dataset info: datasetId={}, error={}", datasetId, e.getMessage());
            return null;
        }
    }
    ...
    public Object postSearchBotAsk(Map<String, Object> config, Object body,
        ...
        // SearchBot Actually Is Dataset Encapsulation of retrieval, or unpublished API?
        // Assume RAGFlow No explicit /searchbots Interface for SDK Call, but Dataset Retrieval Or Chat.
        // But based on BotDTO, it is /api/v1/searchbots/ask (Assume)
        // Here config May be for override, or we just use adapter Instance existing client.
        // but Bot May use different API Key? usually Adapter Instance bound one Key.
        // If Bot Use System KeyThen use directly getClient().

        // Temporarily Assume endpoint /api/v1/searchbots/ask Exists (or similar)
        // If streaming:
        try {
    ...
    public void postAgentBotCompletion(Map<String, Object> config, String agentId, Object body,
            Consumer<String> onData) {
        // AgentBot Correspond /api/v1/agentbots/{id}/completions
        try {
            getClient().postStream("/api/v1/agentbots/" + agentId + "/completions", body, onData);
        } catch (Exception e) {
            log.error("AgentBot Completion Fail", e);
            throw convertToRenException(e);
        }
    }

    // Reuse original auxiliary parse method, keep compatible
    // [Bug Fix] No longer swallow deserializationExceptionAvoid upper-layer misjudgment"Document deleted"
    private PageData<KnowledgeFilesDTO> parseDocumentListResponse(Object dataObj, long curPage, long pageSize) {
        ...
        if (documents == null || documents.isEmpty()) {
            // RAGFlow Clearly returned emptyDocumentList, this is legal"Vacuum"
            return new PageData<>(new ArrayList<>(), 0);
        }
        ...
        for (Object docObj : documents) {
            try {
                // singleDocumentConversion fault tolerance: oneDocumentDeserialization failure does not affect othersDocument
                ...
                log.warn("[RAGFlow] Single document DTO conversion failed, skip this document: {}", e.getMessage());
            }
        }
    ...
    private KnowledgeFilesDTO parseUploadResponse(Object dataObj, String datasetId, MultipartFile file) {
        ...
        // Try fromResponse dataExtract fromDocument ID (documentId)
        if (dataObj != null) {
            try {
                ...
                log.warn("Parse upload response data failed: {}", e.getMessage());
            }
        }

        if (result == null) {
            log.error("Failed to extract documentId from RAGFlow response, response content: {}", dataObj);
            // Here should return minimized containing basicInfoof DTO Rather than nullPrevent upstream NPE
    ...
    /**
     * will RAGFlow strong type InfoVO Map to internal use KnowledgeFilesDTO
     * Ensure all available fields (Namesize,Status, config, etc.) all get full sync
     */
    private KnowledgeFilesDTO mapToKnowledgeFilesDTO(DocumentDTO.InfoVO info, String datasetId) {
        ...
        dto.setError(info.getProgressMsg()); // Set progressDescriptionMap toError infoPrompt
