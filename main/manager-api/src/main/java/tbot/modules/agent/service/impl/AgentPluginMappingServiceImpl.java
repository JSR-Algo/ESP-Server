...
import tbot.modules.agent.dao.AgentPluginMappingMapper;
...
/**
 * @description For Table [ai_agent_plugin_mapping(Unique mapping table between Agent and plugin)Database operation for ]ServiceImplement
    ...
    public List<AgentPluginMapping> agentPluginParamsByAgentId(String agentId) {
        ...
        if (knowledgeBaseMap.size() > 0) {
            for (String pluginCode : knowledgeBaseMap.keySet()) {
                ...
                String description = "If user asks about ["
                        + String.join(",", knowledgeBaseList.stream().map(KnowledgeBaseEntity::getName).toList())
                        + "]covered subject scope related content should call this method, used to query:" + String.join(",",
