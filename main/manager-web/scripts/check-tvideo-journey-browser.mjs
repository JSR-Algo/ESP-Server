...
try {
  ...
  const result = await evaluate(`(async()=>{
    ...
    await window.__SET_TEST_LOCALE__('keys');
    ...
    Object.assign(e,{editorDestroying:false,promptDirty:false,dirtyStepKeys:{},selectedStepDrafts:{},selectedAssetDrafts:{},stepDialogVisible:false,renameVisible:false,sharedImpactVisible:false,sharedImpactReconciling:false,savingLessonVisuals:false,pendingLessonVisualPair:null,validating:false,previewing:false,rebindingSharedVisual:false,reordering:false,addingStep:false,deletingStepKey:null,renaming:false,publishing:false,publishPreparing:false,assetMutationTokens:{},savingStepKeys:{},tvideoJourneyDirty:false,tvideoJourneySaving:false});
    ...
    const proofButtons=()=>{const buttons=[...document.querySelectorAll('.right-operations button')];return['lesson.validate','lesson.previewManifest','lesson.publish'].map(key=>buttons.find(button=>button.textContent.includes(key)))};
