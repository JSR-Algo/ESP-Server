...
try {
  ...
  const result = await evaluate(`(async()=>{
    ...
    e.tvideoJourneyResponse={...e.tvideoJourneyResponse,publishReady:false};await tick();const gateFalse=e.canPublishCurrentProof();e.tvideoJourneyResponse={...e.tvideoJourneyResponse,publishReady:true};e.flattenedDerivativeStatus=null;e.flattenedDerivativeCurrentSourceRevision=null;await tick();const gateTrue=e.canPublishCurrentProof();
    ...
    const legacyV4DerivativeBefore=t.calls.flattenedDerivativeLoads.length;await e.$router.push({path:'/',query:{lessonId:'legacy-v4'}});await wait(()=>e.lesson&&e.lesson.lessonId==='legacy-v4'&&!e.loading&&!e.tvideoJourneyLoading);for(let index=0;index<10;index+=1)await tick();const legacyV4=[t.calls.flattenedDerivativeLoads.length-legacyV4DerivativeBefore,t.calls.flattenedDerivativeLoads.at(-1)?.lessonId,Boolean(document.querySelector('[data-testid="flattened-derivative-readiness"]'))];
    const failedLegacyV4DerivativeBefore=t.calls.flattenedDerivativeLoads.length;await e.$router.push({path:'/',query:{lessonId:'legacy-v4-fail'}});await wait(()=>e.lesson&&e.lesson.lessonId==='legacy-v4-fail'&&!e.loading&&!e.tvideoJourneyLoading);for(let index=0;index<10;index+=1)await tick();const failedLegacyV4=[t.calls.flattenedDerivativeLoads.length-failedLegacyV4DerivativeBefore,t.calls.flattenedDerivativeLoads.at(-1)?.lessonId,Boolean(document.querySelector('[data-testid="flattened-derivative-readiness"]')),e.tvideoJourneyError.includes('journey unavailable')];
    const journeyLoadsBefore=t.calls.journeyLoads.length,derivativeLoadsBefore=t.calls.flattenedDerivativeLoads.length;await e.$router.push({path:'/',query:{lessonId:'journey-v4'}});await wait(()=>e.lesson&&e.lesson.lessonId==='journey-v4'&&!e.loading&&!e.tvideoJourneyLoading);for(let index=0;index<10;index+=1)await tick();const routeReload=[t.calls.journeyLoads.length-journeyLoadsBefore,t.calls.journeyLoads.at(-1),e.tvideoJourneyDirty,Boolean(e.tvideoJourneyPreset),Boolean(e.tvideoJourneyDraft),Boolean(document.querySelector('[data-testid="tvideo-journey-editor"]')),t.calls.flattenedDerivativeLoads.length-derivativeLoadsBefore,Boolean(document.querySelector('[data-testid="flattened-derivative-readiness"]'))];
    return{legacy,localized,unconfigured,hardcodedVisible,tabStates,rawTabKeys,pinnedAssets,keyboard,picker,path,branch,robotState,robotJourneyStates,cueBinding,saveSuccess,saveError,dirtyProof,savingProof,restoredProof,gate:[gateFalse,gateTrue],routeReset,legacyV4,failedLegacyV4,routeReload,runtimeText:document.body.innerText.slice(0,200)};
  })()`);
  ...
  assert.deepEqual(result.legacyV4, [1, 'legacy-v4', true]);
  assert.deepEqual(result.failedLegacyV4, [1, 'legacy-v4-fail', true, true]);
  assert.deepEqual(result.routeReload, [1, 'journey-v4', false, true, true, true, 0, false]);
