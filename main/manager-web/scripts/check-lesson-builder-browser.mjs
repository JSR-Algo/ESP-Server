...
try {
  ...
  const result = await evaluate(`(async()=>{
    ...
    const mp4VideoPreviews=videos.length===4&&videos.every(video=>video.muted&&video.hasAttribute('playsinline')&&video.getAttribute('preload')==='metadata'&&/\.mp4$/.test(video.getAttribute('src')));
    ...
    [...document.querySelectorAll('[data-slot="robotOverlay"] .asset-tile__select')].at(-1).click();await waitFor(()=>!e.cinematicRefSaving);await tick();
    const exactVersionSet=t.calls.visualRefSets.at(-1),robotSelectionHydrated=e.selectedVisualVersionId('robotOverlay'),robotTileSelected=Boolean(document.querySelector('[data-slot="robotOverlay"] .asset-tile.selected'));
  ...
  assert.deepEqual(result.exactVersionSet, { lessonId: 'lesson-1', stepKey: 's2', slot: 'robotOverlay', assetVersionId: 'robot-v5' });
  assert.equal(result.robotSelectionHydrated, 'robot-v5'); assert.equal(result.robotTileSelected, true); assert.equal(result.publishedImmutable, true); assert.equal(result.warnings.length, 1);
  const expectedVisualRefs = [{ slot: 'backgroundScene', assetVersionId: 'scene-v3' }, { slot: 'teachingObject', assetVersionId: 'teach-v2' }, { slot: 'robotOverlay', assetVersionId: 'robot-v5' }];
