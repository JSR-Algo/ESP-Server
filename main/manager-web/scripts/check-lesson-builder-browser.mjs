...
try {
  ...
  const result = await evaluate(`(async()=>{
    ...
    return{selected:e.selectedStepIndex,filters:t.calls.visualFilters,patch:t.calls.update[0],failedPatch:t.calls.update[1],updateCount:t.calls.update.length,metrics:t.validation.budgets.espTft.metrics,preview:[preview.stepIndex,preview.manifest.manifestVersion,preview.manifest.profile,preview.initialPath,e.previewPath.path],cinematicVisible,exactVisible,rendererContractVisible,capabilityVisible,truthfulExactLabel,stateControls,degradedControls,readyBeforeEdit,staleAfterEdit,staleAfterFailure,selectedAfterReload,selectedTilePersisted,errors:t.calls.errors,previewClearedOnEdit,deferredValidationIgnored,newerDraftDirty,newerDraftWord,validateBeforeSaveIgnored,validationIssuesRendered}
  })()`);
