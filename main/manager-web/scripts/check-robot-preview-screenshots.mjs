...
const temp = await mkdtemp(join(tmpdir(), 'tbot-component-preview-'));
...
const chrome = spawn(chromeBin, ['--headless', '--disable-gpu', '--hide-scrollbars', '--force-device-scale-factor=1', '--remote-debugging-port=0', `--user-data-dir=${profileDir}`, 'about:blank'], { stdio: 'ignore' });

async function waitForFile(path, timeoutMs = 10000) {
  const started = Date.now();
  while (Date.now() - started < timeoutMs) { if (existsSync(path)) return readFile(path, 'utf8'); await new Promise((resolve) => setTimeout(resolve, 50)); }
...
socket.on('message', (raw) => { const message = JSON.parse(raw); if (message.id && pending.has(message.id)) { const { resolve, reject } = pending.get(message.id); pending.delete(message.id); message.error ? reject(new Error(message.error.message)) : resolve(message.result); } });
...
async function waitReady() { for (let i = 0; i < 100; i += 1) { if (await evaluate('Boolean(window.__ROBOT_PREVIEW_READY__)')) return; await new Promise((resolve) => setTimeout(resolve, 50)); } throw new Error('Mounted RobotLessonPreview did not become ready'); }
async function settleVisuals() {
  await evaluate(`(async()=>{await Promise.all([...document.images].map(img=>img.complete?Promise.resolve():new Promise((resolve,reject)=>{img.addEventListener('load',resolve,{once:true});img.addEventListener('error',reject,{once:true})})));if(document.fonts)await document.fonts.ready;await Promise.all(document.getAnimations().map(animation=>animation.finished.catch(()=>{})));await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));return true})()`);
}
...
try {
  ...
  for (const minutes of [3, 5, 8]) {
    ...
    const labels = await evaluate(`(async()=>{const out=[];for(const b of document.querySelectorAll('.preview-toolbar button')){b.click();await new Promise(r=>setTimeout(r,0));const robot=document.querySelector('.layer-robotOverlay');out.push([b.textContent.trim(),document.querySelector('.preview-toolbar button.selected').textContent.trim(),document.querySelector('.motion-timeline li:nth-child(2) span').textContent,robot?.className||'missing',robot?.getAnimations().some(animation=>animation.playState==='running')||false]);}return out})()`);
    assert.equal(labels.length, 6); labels.forEach(([label, selected, timeline, motion, animating]) => { assert.equal(selected, label); assert.match(timeline, /^Slave command: /); assert.match(motion, /motion-/); assert.equal(animating, true); });
  ...
  socket.close(); chrome.kill('SIGTERM'); server.close(); await rm(temp, { recursive: true, force: true });
}
