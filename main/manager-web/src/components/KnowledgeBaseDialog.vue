...
export default {
  ...
  watch: {
    visible(val) {
      ...
      if (val) {
        // Load when dialog displayedRAGModel list
        this.loadRAGModels();

        // If new knowledge base and not setragModelIdthen default select firstRAGModel
    ...
    // ListenRAGModel list changed, ensure default value set correctly when adding
    ragModels(newModels) {
  ...
  methods: {
    handleClose() {
      // Do not reset form fields, so previous selections can be kept during edit
      // Reset validation only when dialog closesStatus
      if (this.$refs.knowledgeBaseForm) {
        this.$refs.knowledgeBaseForm.clearValidate();
      }
    ...
    loadRAGModels() {
      if (this.ragModels.length > 0) {
        return; // Already loaded, avoid repeated loading
      }
