...
export default {
    ...
    methods: {
        ...
        async handleFileChange(file) {
            ...
            // Enter step two first,EnsureDOMRendered
            this.currentStep = 2;

            // WaitDOMLoad audio after update
            await this.$nextTick();
        ...
        drawWaveform() {
            ...
            if (this.waveformData.length === 0) {
                console.error('Waveform data empty');
                return;
            }
            ...
            this.waveformData.forEach((value, index) => {
                // Normalize and scale, use80%Height of
        ...
        async uploadAudio() {
            ...
            // Validate audio duration (8-60seconds)
            if (this.audioBuffer) {
                const duration = this.audioBuffer.duration;
