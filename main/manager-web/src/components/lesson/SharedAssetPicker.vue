<template>
  <section class="asset-picker">
    ...
    <div class="asset-picker__grid">
      <button v-for="asset in filtered" :key="asset.assetKey + ':' + (asset.version || '')" type="button" :disabled="disabled" :class="['asset-tile', { selected: selectedKey === asset.assetKey }]" @click="selectAsset(asset)">
...
export default {
  ...
  methods: {
    selectAsset(asset) { if (!this.disabled) this.$emit('select-intent', asset); },
