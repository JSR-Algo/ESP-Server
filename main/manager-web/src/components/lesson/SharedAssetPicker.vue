  ...
  <section class="asset-picker">
    ...
    <div class="asset-picker__grid">
      <div v-for="asset in filtered" :key="asset.assetKey + ':' + (asset.version || '')" :class="['asset-tile', { selected: selectedKey === asset.assetKey }]">
        ...
        <span class="asset-tile__actions"><button type="button" :disabled="disabled" @click="$emit('inspect', asset)">Inspect</button><button type="button" :disabled="disabled" @click="$emit('clone', asset)">Clone</button></span>
      </div>
...
export default {
  ...
  props: { assets: { type: Array, default: () => [] }, selectedKey: { type: String, default: '' }, category: { type: String, default: '' }, disabled: { type: Boolean, default: false } },
  data: () => ({ query: '' }),
...
.asset-picker__grid { display:flex; gap:9px; margin-top:10px; overflow-x:auto; }.asset-tile { background:#fff; border:2px solid transparent; border-radius:12px; display:grid; flex:0 0 145px; gap:4px; padding:7px; text-align:left; }.asset-tile.selected { border-color:#e6a62c; }.asset-tile__select { background:transparent;border:0;cursor:pointer;display:grid;gap:4px;padding:0;text-align:left;width:100%}.asset-tile__select:disabled { cursor:not-allowed; opacity:.55; }.asset-tile__preview { align-items:center; background:#edf2ef; border-radius:8px; display:flex; height:68px; justify-content:center; overflow:hidden; }.asset-tile__preview img { height:100%; object-fit:cover; width:100%; }.asset-tile small { color:#7c8582; }.asset-tile__actions{display:flex;gap:4px}.asset-tile__actions button{background:#edf2ef;border:0;border-radius:7px;color:#31524a;cursor:pointer;flex:1;font-size:11px;padding:5px}.asset-tile__actions button:disabled{cursor:not-allowed;opacity:.55}.empty { color:#909399; padding:16px 0; }
</style>
