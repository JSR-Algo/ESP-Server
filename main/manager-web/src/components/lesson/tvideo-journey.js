...
export function simulateConversationBranch(branch, step = {}, nextStep = null) {
  ...
  if (selected === 'target') return { ...shared, inputClass: 'target word', nextIntent: nextStep ? 'bridge_next_word' : 'celebrate_complete', question: nextStep ? nextStep.teachingCopy.intro : '', bridge: nextStep ? `Let’s connect ${target} to ${nextStep.targetWord}.` : 'You completed the farm journey.', coaching: 'Confirm the child’s contribution.', cueId: `${step.stepKey}-celebrate`, effect: 'celebrate', progressResult: 'advance' };
  if (selected === 'meaning_vi') return { ...shared, inputClass: 'Vietnamese meaning', nextIntent: 'bridge_to_english', question, bridge: `${firstMeaning} connects to the English word ${target}.`, coaching: `Invite the child to say ${target}.`, cueId: `${step.stepKey}-teach`, effect: 'teach', progressResult: 'stay' };
  if (selected === 'related') return { ...shared, inputClass: 'related concept', nextIntent: 'connect_related_concept', question, bridge: `${firstRelated} is connected to ${target}.`, coaching: `Build on the child’s idea, then model ${target}.`, cueId: `${step.stepKey}-thinking`, effect: 'thinking', progressResult: 'stay' };
  if (selected === 'silence') return { ...shared, inputClass: 'silence', nextIntent: 'gentle_reprompt', question, bridge: 'Give the child space, then offer one concrete visual cue.', coaching: `Point to the teaching object and invite ${target}.`, cueId: `${step.stepKey}-listen`, effect: 'listen', progressResult: 'stay' };
  if (selected === 'uncertain') return { ...shared, inputClass: 'uncertain contribution', nextIntent: 'clarify_with_choice', question, bridge: 'Acknowledge the contribution and narrow the prompt.', coaching: `Ask whether the object is ${target}, then model it gently.`, cueId: `${step.stepKey}-thinking`, effect: 'thinking', progressResult: 'stay' };
  return { ...shared, inputClass: `coaching level ${level}`, nextIntent: `coach_level_${level}`, question, bridge: `Keep the child engaged with coaching level ${level}.`, coaching: level === 1 ? `Repeat ${target} slowly.` : level === 2 ? `Segment the approved sounds for ${target}.` : `Model ${target}, use the Vietnamese guidance, and invite one more try.`, cueId: `${step.stepKey}-retry-level-${level}`, effect: `retry-level-${level}`, progressResult: 'stay' };
}
