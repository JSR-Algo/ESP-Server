  ...
  methods: {
    ...
    selectionCellClassName({ row, column, rowIndex, columnIndex }) {
      ...
      if (rowIndex === undefined) {
        // UsesetTimeoutEnsureDOMAlready rendered complete
        setTimeout(() => {
    ...
    handleModelSave({ provideCode, formData, done }) {
      ...
      if (this.editModelData.duplicateMode) {
        ...
        Api.model.addModel({ modelType, provideCode, formData }, ({ data }) => {
          ...
          done && done(); // CalldoneCallback close loadingStatus
        });
      } else {
    ...
    // ProcessEnable/DisableStatusChange
    handleStatusChange(model) {
      ...
      Api.model.updateModelStatus(model.id, newStatus, ({ data }) => {
        if (data.code === 0) {
          ...
          // Operation failedRestore original whenStatus
...
::v-deep .el-table .custom-selection-header .cell::before {
  ...
  text-align: center;
  line-height: 32px;
  /* Set proper row height, ensure complete text display */
  color: black;
  margin-top: 0;
...
.custom-pagination {
  ...
  /* Navigation button style (First page, previous page, next page) */
