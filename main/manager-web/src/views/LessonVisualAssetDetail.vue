...
export default {
  ...
  computed: {
    ...
    targetVersions() { return this.form.mode === 'cloneForLesson' ? this.versions : this.versions.filter((v) => v.versionId !== this.form.sourceVersionId); },
    ...
    replacementReady() { const ids = Array.isArray(this.form.lessonIds) ? this.form.lessonIds : [this.form.lessonIds]; const versionsValid = this.form.mode === 'cloneForLesson' || this.form.sourceVersionId !== this.form.targetVersionId; return Boolean(this.form.sourceVersionId && this.form.targetVersionId && versionsValid && replacementSelectionIsValid(this.usages, this.form.mode, ids)); },
  ...
  watch: {
    'form.sourceVersionId'(value, previous) { if (previous && value !== previous) { this.clearReviewIfClosed(); this.form.lessonIds = this.form.mode === 'cloneForLesson' ? this.routeCloneLessonId : []; if (this.form.mode !== 'cloneForLesson' && this.form.targetVersionId === value) this.form.targetVersionId = (this.versions.find((v) => v.versionId !== value) || {}).versionId || ''; this.load(value); } },
    'form.targetVersionId'() { this.clearReviewIfClosed(); },
    'form.mode'(mode) { this.clearReviewIfClosed(); this.form.lessonIds = mode === 'cloneForLesson' ? this.routeCloneLessonId : []; if (mode !== 'cloneForLesson') this.routeCloneLessonId = ''; if (mode === 'cloneForLesson' && !this.form.targetVersionId) this.form.targetVersionId = this.form.sourceVersionId; if (mode !== 'cloneForLesson' && this.form.targetVersionId === this.form.sourceVersionId) this.form.targetVersionId = (this.versions.find((v) => v.versionId !== this.form.sourceVersionId) || {}).versionId || ''; },
