<template>
  <div class="detail-page">
    ...
    <main v-loading="loading">
      <el-button data-testid="visual-detail-back" type="text" icon="el-icon-arrow-left" @click="$router.push({ name: 'LessonVisualLibrary' })">{{ $t('visual.libraryTitle') }}</el-button>
      ...
      <section data-testid="visual-detail-facts" class="facts">
      ...
      <el-row :gutter="18">
        <el-col :md="14"><el-card shadow="never"><h2>{{ $t('visual.derivativeComparison') }}</h2><el-table data-testid="visual-detail-comparison" :data="comparisonRows"><el-table-column prop="label" :label="$t('visual.metric')" /><el-table-column prop="source" :label="$t('visual.source')" /><el-table-column prop="robot" :label="$t('visual.robotDerivative')" /></el-table></el-card></el-col>
        <el-col :md="10"><el-card shadow="never"><h2>{{ $t('visual.replaceTitle') }}</h2>
          <el-form label-position="top"><el-form-item :label="$t('visual.sourceVersion')"><el-select data-testid="visual-detail-source-version" v-model="form.sourceVersionId" style="width:100%"><el-option v-for="v in versions" :key="v.versionId" :label="versionLabel(v)" :value="v.versionId" /></el-select></el-form-item>
          <el-form-item :label="$t('visual.targetVersion')"><el-select data-testid="visual-detail-target-version" v-model="form.targetVersionId" style="width:100%"><el-option v-for="v in targetVersions" :key="v.versionId" :label="versionLabel(v)" :value="v.versionId" /></el-select></el-form-item>
          <el-form-item :label="$t('visual.mode')"><el-radio-group data-testid="visual-detail-replacement-mode" v-model="form.mode"><el-radio-button v-for="mode in modes" :key="mode" :label="mode" /></el-radio-group></el-form-item>
          <el-form-item v-if="form.mode !== 'global'" :label="$t('visual.affectedLessons')"><el-select data-testid="visual-detail-lessons" v-model="form.lessonIds" :multiple="form.mode === 'selectedLessons'" filterable style="width:100%" :placeholder="$t('visual.lessonIdsHint')"><el-option v-for="lesson in replacementLessons" :key="lesson.lessonId" :label="lessonLabel(lesson)" :value="lesson.lessonId" /></el-select></el-form-item>
...
import { buildReplacementRequest, compareAssetVersions, lessonReplacementOptions, replacementNeedsImpact, replacementSelectionIsValid, replacementTargetVersions, REPLACEMENT_MODES, uniqueAffectedLessons } from '@/utils/lessonVisualLibraryState.mjs';
export default {
  name: 'LessonVisualAssetDetail', components: { HeaderBar, AssetImpactDialog },
  data: () => ({ loading: false, submitting: false, loadSequence: 0, routeCloneLessonId: '', reviewedRequest: null, assetDetail: {}, versions: [], usages: [], modes: REPLACEMENT_MODES, impactVisible: false, impact: {}, form: { sourceVersionId: '', targetVersionId: '', mode: 'global', lessonIds: [] } }),
  computed: {
    ...
    targetVersions() { return replacementTargetVersions(this.versions, this.source, this.form.mode); },
    ...
    replacementReady() { const ids = Array.isArray(this.form.lessonIds) ? this.form.lessonIds : [this.form.lessonIds]; const targetValid = this.targetVersions.some((v) => v.versionId === this.form.targetVersionId); return Boolean(this.form.sourceVersionId && this.form.targetVersionId && targetValid && replacementSelectionIsValid(this.usages, this.form.mode, ids)); },
  ...
  watch: {
    'form.sourceVersionId'(value, previous) { if (previous && value !== previous) { this.clearReviewIfClosed(); this.form.lessonIds = this.form.mode === 'cloneForLesson' ? this.routeCloneLessonId : []; const source = this.versions.find((v) => v.versionId === value) || {}; const targets = replacementTargetVersions(this.versions, source, this.form.mode); if (!targets.some((v) => v.versionId === this.form.targetVersionId)) this.form.targetVersionId = (targets[0] || {}).versionId || ''; this.load(value); } },
    'form.targetVersionId'() { this.clearReviewIfClosed(); },
    'form.mode'(mode) { this.clearReviewIfClosed(); this.form.lessonIds = mode === 'cloneForLesson' ? this.routeCloneLessonId : []; if (mode !== 'cloneForLesson') this.routeCloneLessonId = ''; const targets = replacementTargetVersions(this.versions, this.source, mode); if (!targets.some((v) => v.versionId === this.form.targetVersionId)) this.form.targetVersionId = (targets[0] || {}).versionId || ''; },
    'form.lessonIds': { deep: true, handler() { this.clearReviewIfClosed(); } },
    '$route.query': { deep: true, handler(query) { this.applyRouteIntent(query); } },
  },
  mounted() { this.applyRouteIntent((this.$route && this.$route.query) || {}); this.load(); },
  methods: {
    applyRouteIntent(query = {}) { if (query.mode !== 'cloneForLesson' || typeof query.lessonId !== 'string' || !query.lessonId.trim()) return; this.routeCloneLessonId = query.lessonId.trim(); this.form.mode = 'cloneForLesson'; this.form.lessonIds = this.routeCloneLessonId; this.form.targetVersionId = this.form.sourceVersionId; },
    load(sourceVersionId) { const sequence = ++this.loadSequence; this.loading = true; Api.lesson.getVisualAssetDetail(this.assetKey, { sourceVersionId }, ({ asset, versions, usages, sourceVersionId: selected }) => { if (sequence !== this.loadSequence) return; this.assetDetail = asset; this.versions = versions.sort((a, b) => b.version - a.version); this.usages = usages; const source = this.versions.find((v) => v.versionId === selected) || this.versions[0] || {}; this.form.sourceVersionId = source.versionId || ''; const targets = replacementTargetVersions(this.versions, source, this.form.mode); if (!targets.some((v) => v.versionId === this.form.targetVersionId)) this.form.targetVersionId = (targets[0] || {}).versionId || ''; this.loading = false; }, (m) => { if (sequence !== this.loadSequence) return; this.loading = false; this.$message.error(m || this.$t('visual.loadFail')); }); },
