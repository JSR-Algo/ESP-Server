<template>
  <div class="welcome">
    ...
    <!-- Add new/Edit knowledge base dialog -->
...
export default {
  ...
  methods: {
    ...
    fetchKnowledgeBaseList: function() {
      ...
      Api.knowledgeBase.getKnowledgeBaseList(
        ...
        (res) => {
          ...
          // Fix: from res.data GetPaged dataInstead of res.data.data
          // Because knowledgeBase.js Passed whole directlyResponseObject
    ...
    showViewDialog: function(row) {
      // Go to upload file page, passKnowledge base IDandName
      this.$router.push({
    ...
    handleSubmit: function(form) {
      ...
      if (form.id) {
        ...
        // Add new - Only pass necessary fields, not passid
        const createData = {
          name: form.name,
    ...
    handleStatusChange: function(row) {
      // Only pass fields needing update, ensure includeidField
      const updateForm = {
        id: row.id, // AddidField, backend needs this field to locate record
        datasetId: row.datasetId,
        name: row.name,
        description: row.description,
        status: row.status
      ...
      Api.knowledgeBase.updateKnowledgeBase(row.datasetId, updateForm, (res) => {
        ...
        if (res.data && res.data.code !== 0) {
          // Restore originalStatus
          this.fetchKnowledgeBaseList();
          this.$message.error(res.data?.msg || this.$t('knowledgeBaseManagement.updateFailed'));
        } else {
          // Update success, show successMessage
          this.$message.success(this.$t('knowledgeBaseManagement.updateSuccess'));
        }
      }, () => {
        // Restore originalStatus
