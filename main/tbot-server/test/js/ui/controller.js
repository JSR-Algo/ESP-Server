...
class UIController {
    ...
    initEventListeners() {
        ...
        if (cameraBtn) {
            cameraBtn.addEventListener('click', () => {
                ...
                cameraTimer = setTimeout(() => {
                    ...
                    if (!cameraContainer) {
                        log('camera containerDoes not exist', 'warning');
                        return;
                    }
                        ...
                        if (typeof window.startCamera === 'function') {
                            window.startCamera().then(success => {
                                if (success) {
                                    ...
                                    this.addChatMessage('⚠️ cameraStart Failed, pleaseCheckBrowserpermission', false);
                                }
                            }).catch(error => {
                                log(`Startcamera exception: ${error.message}`, 'error');
                            });
                        } else {
                            log('startCameraFunctionnot yetDefine', 'warning');
                        }
                    }
                }, 300);
            });
        ...
        // Clickmodal backgroundClose(only for specific modalDisablethisFunction)
        const modals = document.querySelectorAll('.modal');
        modals.forEach(modal => {
            modal.addEventListener('click', (e) => {
                if (e.target === modal) {
                    // settingsModal,mcpToolModal,mcpPropertyModal only canPassClickXClose
                    const nonClosableModals = ['settingsModal', 'mcpToolModal', 'mcpPropertyModal'];
                    if (nonClosableModals.includes(modal.id)) {
                        return; // forbidClickbackgroundClose
                    }
                    this.hideModal(modal.id);
                }
            });
    ...
    updateDialButton(isConnected) {
        ...
        if (cameraBtn && !isConnected) {
            ...
            cameraBtn.title = 'please firstConnectServer';
            // Closecamera
            if (typeof window.stopCamera === 'function') {
                window.stopCamera();
            }
        ...
        if (cameraBtn && isConnected) {
            if (window.cameraAvailable) {
                ...
                cameraBtn.title = 'open/Closecamera';
            } else {
                cameraBtn.disabled = true;
        ...
        if (recordBtn) {
            ...
            if (isConnected && microphoneAvailable) {
                ...
                recordBtn.title = 'Startrecording';
                // Restore record button to normal state
                recordBtn.querySelector('.btn-text').textContent = 'recording';
                recordBtn.classList.remove('recording');
            } else {
                ...
                if (!microphoneAvailable) {
                    recordBtn.title = window.isHttpNonLocalhost ? 'Currentbecause ishttpAccess, cannot record audio, can only use text interaction' : 'microphone unavailable';
                } else {
                    recordBtn.title = 'please firstConnectServer';
                }
                // Restore record button to normal state
                recordBtn.querySelector('.btn-text').textContent = 'recording';
                recordBtn.classList.remove('recording');
    ...
    updateMicrophoneAvailability(isAvailable, isHttpNonLocalhost) {
        ...
        if (!isAvailable) {
            ...
            recordBtn.querySelector('.btn-text').textContent = 'recording';
            recordBtn.title = isHttpNonLocalhost ? 'Currentbecause ishttpAccess, cannot record audio, can only use text interaction' : 'microphone unavailable';
    ...
    switchLive2DModel() {
        const modelSelect = document.getElementById('live2dModelSelect');
        if (!modelSelect) {
            console.error('ModelSelectdropdownDoes not exist');
            return;
        }
        ...
        if (app && app.live2dManager) {
            app.live2dManager.switchModel(selectedModel)
                .then(success => {
                    if (success) {
                        this.addChatMessage(`alreadySwitchtoModel: ${selectedModel}`, false);
                    } else {
                        this.addChatMessage('ModelSwitchFail', false);
                    }
                })
                .catch(error => {
                    console.error('ModelSwitcherror:', error);
                    this.addChatMessage('ModelSwitcherror occurred', false);
                });
        } else {
            this.addChatMessage('Live2DManagedevice not yetInitialize', false);
        }
    }
    ...
    startAIChatSession() {
        this.addChatMessage('ConnectSuccess,Startlet's chat~😊', false);
        // Check microphone availability and show error messages if needed
        if (!window.microphoneAvailable) {
            if (window.isHttpNonLocalhost) {
                this.addChatMessage('⚠️ Currentbecause ishttpAccess, cannot record audio, can only use text interaction', false);
            } else {
                this.addChatMessage('⚠️ microphone unavailable, pleaseCheck PermissionSet, can only use text interaction', false);
            }
        }
        ...
        if (window.cameraAvailable && typeof window.startCamera === 'function') {
            window.startCamera().then(success => {
                if (success) {
                    ...
                    this.addChatMessage('⚠️ cameraStart Failed, may beBrowserdenied', false);
                }
            }).catch(error => {
                log(`Startcamera exception: ${error.message}`, 'error');
            });
    ...
    async handleConnect() {
        ...
        if (!otaUrlInput || !otaUrlInput.value) {
            this.addChatMessage('EnterOTAServerAddress', false);
            return;
        }
        ...
        if (dialBtn) {
            dialBtn.classList.add('dial-active');
            dialBtn.querySelector('.btn-text').textContent = 'Connectmid...';
            dialBtn.disabled = true;
        }

        // Show connecting message
        this.addChatMessage('runningConnectServer...', false);
        ...
        try {
            ...
            if (isConnected) {
                ...
                throw new Error('OTAConnection Failed');
            }
        } catch (error) {
            ...
            const errorMessage = error.message.includes('Cannot set properties of null')
                ? 'Connection Failed: pleaseCheckDeviceConnect'
    ...
    addMCPTool() {
        ...
        toolDiv.innerHTML = `
            <div class="property-item">
                <input type="text" placeholder="Tool name" value="newTool">
                <input type="text" placeholder="ToolDescription" value="ToolDescription">
