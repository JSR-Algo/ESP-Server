...
export class AudioRecorder {
    ...
    async createAudioProcessor() {
        ...
        try {
            if (this.audioContext.audioWorklet) {
                ...
                log('UseAudioWorkletProcessAudio', 'success');
                ...
                log('AudioWorkletunavailable, useScriptProcessorNodeAsfallback', 'warning');
                return this.createScriptProcessor();
            }
        } catch (error) {
            log(`CreateAudio ProcessingdeviceFail: ${error.message}, try fallback`, 'error');
            return this.createScriptProcessor();
        }
    }
    ...
    createScriptProcessor() {
        try {
            ...
            silent.connect(this.audioContext.destination);
            log('UseScriptProcessorNodeAsfallbackSuccess', 'warning');
            return { node: scriptProcessor, type: 'processor' };
        } catch (fallbackError) {
            log(`fallback alsoFail: ${fallbackError.message}`, 'error');
            return null;
        }
    }
    ...
    encodeAndSendOpus(pcmData = null) {
        if (!this.opusEncoder) {
            log('OpusEncodernot yetInitialize', 'error');
            return;
        }
        try {
            if (pcmData) {
                ...
                if (opusData && opusData.length > 0) {
                    ...
                    if (this.websocket && this.websocket.readyState === WebSocket.OPEN) {
                        try {
                            ...
                            log(`WebSocketSenderror: ${error.message}`, 'error');
                        }
                    }
                } else {
                    log('OpusEncodeFail, notReturnValidData', 'error');
                }
            } else {
            ...
            log(`OpusEncodeerror: ${error.message}`, 'error');
        }
    }
        ...
        try {
            ...
            if (wsHandler && wsHandler.isRemoteSpeaking && wsHandler.currentSessionId) {
                ...
                if (this.websocket && this.websocket.readyState === WebSocket.OPEN) {
                    ...
                    log('alreadySendabort message', 'info');
                }
            }
            if (!this.initEncoder()) {
                log('unableStartrecording: OpusEncoderInitializeFail', 'error');
                return false;
            }
            log('pleaseAt leastrecord1-2secondsAudiotoEnsurecollect enoughData', 'info');
            ...
            if (!processorResult) {
                log('unableCreateAudio Processingdevice', 'error');
                return false;
            }
            ...
            if (this.websocket && this.websocket.readyState === WebSocket.OPEN) {
                log(`alreadySendrecordingStartmessage`, 'info');
            } else {
                log('WebSocketNot connected, unableSendStartmessage', 'error');
                return false;
            }
            // Start visualization
            ...
            log('alreadyStartPCMDirectrecording', 'success');
            return true;
        } catch (error) {
            log(`DirectrecordingStarterror: ${error.message}`, 'error');
            this.isRecording = false;
            return false;
        }
    }
    ...
    stop() {
        ...
        try {
            ...
            if (this.websocket && this.websocket.readyState === WebSocket.OPEN) {
                ...
                log('alreadySendrecordingStopsignal', 'info');
            }
            if (this.onRecordingStop) {
                this.onRecordingStop();
            }
            log('alreadyStopPCMDirectrecording', 'success');
            return true;
        } catch (error) {
            log(`DirectrecordingStoperror: ${error.message}`, 'error');
            return false;
        }
    }
...
export async function checkMicrophoneAvailability() {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
        log('BrowsernotSupportgetUserMedia API', 'warning');
        return false;
    }
    try {
        ...
        log('microphone availabilityCheckSuccess', 'success');
        return true;
    } catch (error) {
