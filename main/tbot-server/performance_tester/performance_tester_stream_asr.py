...
class BaseASRTester:
    ...
    def _calculate_result(self, service_name, latencies, test_count):
        """Calculate test results (fix: handle None values correctly, exclude failed tests)"""
        # RemoveNonevalues (failed tests) and invalid delays. Only count valid delays
        ...
        for i in range(test_count):
            try:
                ...
                async with websockets.connect(
                    ...
                    # Send audio data (use last-frame marker to tell server audio ended)
...
class QwenASRFlashTester(BaseASRTester):
    ...
    async def _test_single(self, audio_file_info):
        ...
        try:
            ...
            # Optimization: move temp file preparation before timing, reduce diskIOImpact on performance test
            ...
            # Unified timing start: atAPIStart timing before call (but file preparation completed)
            start_time = time.time()
    ...
    async def test(self, test_count=5):
        ...
        if not self.asr_config and not os.getenv("DASHSCOPE_API_KEY"):
            return {"name": "Tongyi Qianwen ASR", "latency": 0, "status": "Failed: api_key not configured"}
        ...
        for i in range(test_count):
            try:
                ...
                latencies.append(latency)
                print(f"[Tongyi ASR] Run {i+1} first-word latency: {latency:.3f}s")
            except Exception as e:
    ...
    async def test(self, test_count: int = 5):
        ...
        for k in required:
            if k not in self.asr_config:
                return {"name": "iFlytek streaming ASR", "latency": 0, "status": f"Failed: missing config {k}"}
        ...
        for i in range(test_count):
            try:
                ...
                async with websockets.connect(
                    ...
                    # First frame: remove punc field, avoidUnknownParameterError
                    ...
                    async for message in ws:
                        ...
                        if data.get("code") != 0:
                            raise Exception(f"iFlytekError: {data.get('message')}")
                        ...
                        if ws_result:
                            ...
                            if text.strip() and first_token:
                                latency = time.time() - start_time
                                latencies.append(latency)
                                print(f"[iFlytekASR] No.{i+1}times First word latency: {latency:.3f}s")
                                first_token = False
                                break

            except Exception as e:
                print(f"[iFlytekASR] No.{i+1}test failures: {str(e)}")
                latencies.append(None)
...
class ASRPerformanceSuite:
    ...
    def register_tester(self, tester_class):
        try:
            ...
            print(f"alreadyRegisterTester: {tester.config_key}")
        except Exception as e:
            name_map = {
    ...
    def _print_results(self, test_count):
        if not self.results:
            print("No validASRTest Result")
            return

        print(f"\n{'='*60}")
        print("StreamingASRFirst wordResponseTime test result")
        print(f"{'='*60}")
        print(f"Test count: EachASR serviceTest {test_count} times")
        ...
        print("\nTest notes:")
        print("- Timing start: before connection established (includes handshake, audio sending, and first recognition result receiving full flow)")
        print("- Tongyi Qianwen optimization: temp files prepared before timing to reduce disk IO impact on test")
        print("- Error handling: failed tests are not included in average, only latency of successful tests counted")
        print("- Sorting rule: successes by latency ascending, failures at end")
