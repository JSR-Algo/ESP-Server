import asyncio
import logging
import os
import statistics
import time
import concurrent.futures
from typing import Dict, Optional
import yaml
import aiohttp
from tabulate import tabulate
from core.utils.llm import create_instance as create_llm_instance
from config.settings import load_config

# Set global log level to WARNING, suppress INFO Level Log
logging.basicConfig(level=logging.WARNING)

description = "LLM performance test"


def _is_gateway_error(error_msg: str) -> bool:
    """Gateway failures are upstream availability errors, not latency samples."""
    return (
        "502" in error_msg
        or "bad gateway" in error_msg
        or "error code: 502" in error_msg
    )


class LLMPerformanceTester:
    def __init__(self):
        self.config = load_config()
        # Use test more suited to agent scenarioContent, includeSystem prompt
        self.system_prompt = self._load_system_prompt()
        self.test_sentences = self.config.get("module_test", {}).get(
            "test_sentences",
            [
                "Hello, I feel down today. Can you comfort me?",
                "Help me check tomorrow's weather?",
                "I want to hear an interesting story. Can you tell me one?",
                "What time is it now? What day is today?",
                "I want to set an alarm for 8 AM tomorrow to remind me of meeting",
            ],
        )
        self.results = {}

    def _load_system_prompt(self) -> str:
        """Load system prompt"""
        try:
            prompt_file = os.path.join(
                os.path.dirname(os.path.dirname(__file__)), self.config.get("prompt_template", "agent-base-prompt.txt")
            )
            with open(prompt_file, "r", encoding="utf-8") as f:
                content = f.read()
                # Replace template variables with test values
                content = content.replace(
                    "{{base_prompt}}", "You are TBOT, smart cute AI assistant"
                )
                content = content.replace(
                    "{{emojiList}}", "😀,😃,😄,😁,😊,😍,🤔,😮,😱,😢,😭,😴,😵,🤗,🙄"
                )
                content = content.replace("{{current_time}}", "August 17, 2024 12:30:45")
                content = content.replace("{{today_date}}", "August 17, 2024")
                content = content.replace("{{today_weekday}}", "Saturday")
                content = content.replace("{{lunar_date}}", "Jiachen year, seventh month, fourteenth day")
                content = content.replace("{{local_address}}", "Beijing")
                content = content.replace("{{weather_info}}", "Sunny today, 25-32℃")
                return content
        except Exception as e:
            print(f"Cannot load system prompt file: {e}")
            return "You are TBOT, smart and cute AI assistant. Reply to user in warm, friendly tone."

    def _collect_response_sync(self, llm, messages, llm_name, sentence_start):
        """Helper method for synchronously collecting response data"""
        chunks = []
        first_token_received = False
        first_token_time = None

        try:
            response_generator = llm.response("perf_test", messages)
            chunk_count = 0
            for chunk in response_generator:
                chunk_count += 1
                # Every certain processedQuantityofchunkCheck whether should interrupt
                if chunk_count % 10 == 0:
                    # Exit early by checking whether current thread is marked interrupted
                    import threading

                    if (
                        threading.current_thread().ident
                        != threading.main_thread().ident
                    ):
                        # If not main thread, check whether should stop
                        pass

                # CheckchunkContainsError info
                chunk_str = str(chunk)
                if (
                    "Exception" in chunk_str
                    or "Error" in chunk_str
                    or "502" in chunk_str.lower()
                ):
                    error_msg = chunk_str.lower()
                    print(f"{llm_name} response contains error message: {error_msg}")
                    # Throw one containingError infoofException
                    raise Exception(chunk_str)

                if not first_token_received and chunk.strip() != "":
                    first_token_time = time.time() - sentence_start
                    first_token_received = True
                    print(f"{llm_name} First Token: {first_token_time:.3f}s")
                chunks.append(chunk)
        except Exception as e:
            # More DetailedError info
            error_msg = str(e).lower()
            print(f"{llm_name} response collection exception: {error_msg}")
            # For502ErrororNetwork error, directly throwExceptionLet upper layer handle
            if (
                "502" in error_msg
                or "bad gateway" in error_msg
                or "error code: 502" in error_msg
                or "Exception" in str(e)
                or "Error" in str(e)
            ):
                raise e
            # For OthersErrorCan return partial results
            return chunks, first_token_time

        return chunks, first_token_time

    async def _check_ollama_service(self, base_url: str, model_name: str) -> bool:
        """Async check Ollama service status"""
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(f"{base_url}/api/version") as response:
                    if response.status != 200:
                        print(f"Ollama service not started or inaccessible: {base_url}")
                        return False
                async with session.get(f"{base_url}/api/tags") as response:
                    if response.status == 200:
                        data = await response.json()
                        models = data.get("models", [])
                        if not any(model["name"] == model_name for model in models):
                            print(
                                f"Ollama Model {model_name} Not found, please use first `ollama pull {model_name}` Download"
                            )
                            return False
                    else:
                        print("Cannot Get Ollama Model list")
                        return False
                return True
            except Exception as e:
                print(f"Cannot connect to Ollama Service: {str(e)}")
                return False

    async def _test_single_sentence(
        self, llm_name: str, llm, sentence: str
    ) -> Optional[Dict]:
        """Test single sentence performance"""
        try:
            print(f"{llm_name} Start Test: {sentence[:20]}...")
            sentence_start = time.time()
            first_token_received = False
            first_token_time = None

            # Build ContainsSystem promptofMessage
            messages = [
                {"role": "system", "content": self.system_prompt},
                {"role": "user", "content": sentence},
            ]

            # Useasyncio.wait_forPerform timeout control
            try:
                loop = asyncio.get_event_loop()
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    # CreateResponseCollect Tasks
                    future = executor.submit(
                        self._collect_response_sync,
                        llm,
                        messages,
                        llm_name,
                        sentence_start,
                    )

                    # Useasyncio.wait_forImplement timeout control
                    try:
                        response_chunks, first_token_time = await asyncio.wait_for(
                            asyncio.wrap_future(future), timeout=10.0
                        )
                    except asyncio.TimeoutError:
                        print(f"{llm_name} Test timeout (10seconds), skip")
                        # Force Cancelfuture
                        future.cancel()
                        # Wait short time to ensure thread pool tasks canResponseCancel
                        try:
                            await asyncio.wait_for(
                                asyncio.wrap_future(future), timeout=1.0
                            )
                        except (
                            asyncio.TimeoutError,
                            concurrent.futures.CancelledError,
                            Exception,
                        ):
                            # Ignore AllExceptionEnsure program continues execution
                            pass
                        return None

            except Exception as timeout_error:
                print(f"{llm_name} ProcessException: {timeout_error}")
                return None

            response_time = time.time() - sentence_start
            print(f"{llm_name} CompleteResponse: {response_time:.3f}s")

            return {
                "name": llm_name,
                "type": "llm",
                "first_token_time": first_token_time,
                "response_time": response_time,
            }
        except Exception as e:
            error_msg = str(e).lower()
            # 502 means the upstream provider failed; record it as an error result.
            if _is_gateway_error(error_msg):
                print(f"{llm_name} encountered 502 gateway error; record network failure")
                return {
                    "name": llm_name,
                    "type": "llm",
                    "errors": 1,
                    "error_type": "502Network error",
                }
            print(f"{llm_name} Sentence test failed: {str(e)}")
            return None

    async def _test_llm(self, llm_name: str, config: Dict) -> Dict:
        """Async test single LLM performance"""
        try:
            # For Ollama, skip api_key check and handle specially
            if llm_name == "Ollama":
                base_url = config.get("base_url", "http://localhost:11434")
                model_name = config.get("model_name")
                if not model_name:
                    print("Ollama Not configured model_name")
                    return {
                        "name": llm_name,
                        "type": "llm",
                        "errors": 1,
                        "error_type": "Network error",
                    }

                if not await self._check_ollama_service(base_url, model_name):
                    return {
                        "name": llm_name,
                        "type": "llm",
                        "errors": 1,
                        "error_type": "Network error",
                    }
            else:
                if "api_key" in config and any(
                    x in config["api_key"] for x in ["Your", "placeholder", "sk-xxx"]
                ):
                    print(f"SkipNot configuredof LLM: {llm_name}")
                    return {
                        "name": llm_name,
                        "type": "llm",
                        "errors": 1,
                        "error_type": "ConfigError",
                    }

            # Get actual type (compatible with old config)
            module_type = config.get("type", llm_name)
            llm = create_llm_instance(module_type, config)

            # Use Unified UTF-8 Encode
            test_sentences = [
                s.encode("utf-8").decode("utf-8") for s in self.test_sentences
            ]

            # Create test tasks for all sentences
            sentence_tasks = []
            for sentence in test_sentences:
                sentence_tasks.append(
                    self._test_single_sentence(llm_name, llm, sentence)
                )

            # Run all sentence tests concurrently and handle possibleException
            sentence_results = await asyncio.gather(
                *sentence_tasks, return_exceptions=True
            )

            # Process results, filter outExceptionandNonevalue
            valid_results = []
            for result in sentence_results:
                if isinstance(result, dict) and result is not None:
                    valid_results.append(result)
                elif isinstance(result, Exception):
                    error_msg = str(result).lower()
                    if _is_gateway_error(error_msg):
                        print(f"{llm_name} encountered 502 gateway error; record network failure")
                        return {
                            "name": llm_name,
                            "type": "llm",
                            "errors": 1,
                            "error_type": "502Network error",
                        }
                    else:
                        print(f"{llm_name} Sentence TestException: {result}")

            if not valid_results:
                print(f"{llm_name} No valid data, may encounter network problem or configError")
                return {
                    "name": llm_name,
                    "type": "llm",
                    "errors": 1,
                    "error_type": "Network error",
                }

            # Check valid result count. If too few, test is considered failed
            if len(valid_results) < len(test_sentences) * 0.3:  # At least 30% success rate required
                print(
                    f"{llm_name} Too few successful test sentences({len(valid_results)}/{len(test_sentences)})May be unstable network or interface issue"
                )
                return {
                    "name": llm_name,
                    "type": "llm",
                    "errors": 1,
                    "error_type": "Network error",
                }

            first_token_times = [
                r["first_token_time"]
                for r in valid_results
                if r.get("first_token_time")
            ]
            response_times = [r["response_time"] for r in valid_results]

            # FilterExceptiondata (exceed3data beyond standard deviations)
            if len(response_times) > 1:
                mean = statistics.mean(response_times)
                stdev = statistics.stdev(response_times)
                filtered_times = [t for t in response_times if t <= mean + 3 * stdev]
            else:
                filtered_times = response_times

            return {
                "name": llm_name,
                "type": "llm",
                "avg_response": sum(response_times) / len(response_times),
                "avg_first_token": (
                    sum(first_token_times) / len(first_token_times)
                    if first_token_times
                    else 0
                ),
                "success_rate": f"{len(valid_results)}/{len(test_sentences)}",
                "errors": 0,
            }
        except Exception as e:
            error_msg = str(e).lower()
            if _is_gateway_error(error_msg):
                print(f"LLM {llm_name} encountered 502 gateway error; record network failure")
            else:
                print(f"LLM {llm_name} Test Failed: {str(e)}")
            error_type = "Network error"
            if "timeout" in str(e).lower():
                error_type = "Timeout Connection"
            return {
                "name": llm_name,
                "type": "llm",
                "errors": 1,
                "error_type": error_type,
            }

    def _print_results(self):
        """Print test results"""
        print("\n" + "=" * 50)
        print("LLM Performance test result")
        print("=" * 50)

        if not self.results:
            print("No available test results")
            return

        headers = ["Model name", "AverageResponseTime(s)", "firstTokenTime(s)", "Success rate", "Status"]
        table_data = []

        # Collect all data and classify
        valid_results = []
        error_results = []

        for name, data in self.results.items():
            if data["errors"] == 0:
                # Normal Result
                avg_response = f"{data['avg_response']:.3f}"
                avg_first_token = (
                    f"{data['avg_first_token']:.3f}"
                    if data["avg_first_token"] > 0
                    else "-"
                )
                success_rate = data.get("success_rate", "N/A")
                status = "✅ Normal"

                # SaveUsed forSortValue of
                first_token_value = (
                    data["avg_first_token"]
                    if data["avg_first_token"] > 0
                    else float("inf")
                )

                valid_results.append(
                    {
                        "name": name,
                        "avg_response": avg_response,
                        "avg_first_token": avg_first_token,
                        "success_rate": success_rate,
                        "status": status,
                        "sort_key": first_token_value,
                    }
                )
            else:
                # ErrorResult
                avg_response = "-"
                avg_first_token = "-"
                success_rate = "0/5"

                # Get specific error type
                error_type = data.get("error_type", "Network error")
                status = f"❌ {error_type}"

                error_results.append(
                    [name, avg_response, avg_first_token, success_rate, status]
                )

        # By firstTokenTime AscendingSort
        valid_results.sort(key=lambda x: x["sort_key"])

        # Convert sorted valid results to table data
        for result in valid_results:
            table_data.append(
                [
                    result["name"],
                    result["avg_response"],
                    result["avg_first_token"],
                    result["success_rate"],
                    result["status"],
                ]
            )

        # willErrorAdd result to end of table data
        table_data.extend(error_results)

        print(tabulate(table_data, headers=headers, tablefmt="grid"))
        print("\nTest notes:")
        print("- TestContent: include completeSystem promptagent conversation scenario")
        print("- Timeout control: max wait time for single request is10seconds")
        print("- ErrorProcess: record 502 and network exceptions as failed model results")
        print("- Success rate: successResponseSentence ofQuantity/totalTest sentence countamount")
        print("\nTest complete!")

    async def run(self):
        """Execute full async test"""
        print("Start filtering available LLM Module...")

        # Create all test tasks
        all_tasks = []

        # LLM test task
        if self.config.get("LLM") is not None:
            for llm_name, config in self.config.get("LLM", {}).items():
                # Check config validity
                if llm_name == "CozeLLM":
                    if any(x in config.get("bot_id", "") for x in ["Your"]) or any(
                        x in config.get("user_id", "") for x in ["Your"]
                    ):
                        print(f"LLM {llm_name} Not configured bot_id/user_id, skipped")
                        continue
                elif "api_key" in config and any(
                    x in config["api_key"] for x in ["Your", "placeholder", "sk-xxx"]
                ):
                    print(f"LLM {llm_name} api_key not configured, skipped")
                    continue

                # For Ollama,check service firstStatus
                if llm_name == "Ollama":
                    base_url = config.get("base_url", "http://localhost:11434")
                    model_name = config.get("model_name")
                    if not model_name:
                        print("Ollama Not configured model_name")
                        continue

                    if not await self._check_ollama_service(base_url, model_name):
                        continue

                print(f"Add LLM Test Task: {llm_name}")
                all_tasks.append(self._test_llm(llm_name, config))

        print(f"\nFind {len(all_tasks)} Available LLM Module")
        print("\nStart concurrent testing of all modules...\n")

        # Run all test tasks concurrently, but set independent timeout for each task
        async def test_with_timeout(task, timeout=30):
            """Add timeout protection for each test task"""
            try:
                return await asyncio.wait_for(task, timeout=timeout)
            except asyncio.TimeoutError:
                print(f"Test task timeout ({timeout}seconds), skip")
                return {
                    "name": "Unknown",
                    "type": "llm",
                    "errors": 1,
                    "error_type": "Timeout Connection",
                }
            except Exception as e:
                print(f"Test TaskException: {str(e)}")
                return {
                    "name": "Unknown",
                    "type": "llm",
                    "errors": 1,
                    "error_type": "Network error",
                }

        # Wrap timeout protection for each task
        protected_tasks = [test_with_timeout(task) for task in all_tasks]

        # Run all test tasks concurrently
        all_results = await asyncio.gather(*protected_tasks, return_exceptions=True)

        # Processing Result
        for result in all_results:
            if isinstance(result, dict):
                if result.get("errors") == 0:
                    self.results[result["name"]] = result
                else:
                    # Record even with errors, for showing failure status
                    if result.get("name") != "Unknown":
                        self.results[result["name"]] = result
            elif isinstance(result, Exception):
                print(f"Test result handlingException: {str(result)}")

        # Print result
        print("\nGenerate test report...")
        self._print_results()


async def main():
    tester = LLMPerformanceTester()
    await tester.run()


if __name__ == "__main__":
    asyncio.run(main())
