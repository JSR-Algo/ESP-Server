...
play_music_function_desc = {
    ...
    "function": {
        "name": "play_music",
        "description": "Call when user requests playing music or songs.",
        "parameters": {
            "type": "object",
            "properties": {
                "song_name": {
                    "type": "string",
                    "description": "SongNameIf user did not specify exact song name, then'random', Return music name when explicitly specified Example: ```User: play Two Tigers\nParameters: Two Tigers``` ```User: play music \nParameters: random ```",
                }
            },
            "required": ["song_name"],
        },
...
def play_music(conn: "ConnectionHandler", song_name: str):
    try:
        ...
        if not conn.loop.is_running():
            conn.logger.bind(tag=TAG).error("EventLoop not running, cannot submit task")
            return ActionResponse(
                action=Action.RESPONSE, result="System Busy", response="Please try later"
            )
        ...
        def handle_done(f):
            try:
                f.result()  # Handle success logic here
        ...
        conn.logger.bind(tag=TAG).error(f"Handle music intent error: {e}")
        return ActionResponse(
...
def get_music_files(music_dir, music_ext):
    ...
    for file in music_dir.rglob("*"):
        ...
        if file.is_file():
            ...
            # Determine whether extension is in list
            if ext in music_ext:
                # Add relative path
...
async def handle_music_command(conn: "ConnectionHandler", text):
    ...
    # Try matching specific song title
    if os.path.exists(MUSIC_CACHE["music_dir"]):
        ...
        if potential_song:
            ...
            if best_match:
                ...
                return True
    # Check whether general play music command
    await play_local_music(conn)
    return True


def _get_random_play_prompt(song_name):
    """Generate random playback guide"""
    # RemoveFile extension
    clean_name = os.path.splitext(song_name)[0]
    prompts = [
        f"Playing for you, {clean_name}",
        f"Enjoy song, 《{clean_name}》",
    ...
    # Use Directlyrandom.choice, not setseed
    ...
    try:
        if not os.path.exists(MUSIC_CACHE["music_dir"]):
            conn.logger.bind(tag=TAG).error(
                f"Music directory not exist: " + MUSIC_CACHE["music_dir"]
            )
            return
        ...
        if specific_file:
            ...
            if not MUSIC_CACHE["music_files"]:
                conn.logger.bind(tag=TAG).error("Not foundMP3Music File")
                return
        ...
        conn.logger.bind(tag=TAG).error(f"Play music failed: {str(e)}")
