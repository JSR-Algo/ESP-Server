...
class DeviceIoTExecutor(ToolExecutor):
    ...
    async def execute(
        ...
        try:
            # Parse ToolName, get device name and operation type
            if tool_name.startswith("get_"):
                ...
                if len(parts) >= 3:
                    ...
                    if value is not None:
                        ...
                        response_failure = arguments.get(
                            "response_failure", f"Unable to get {device_name} status"
                        )
                        return ActionResponse(
                ...
                if len(parts) >= 2:
                    ...
                    # Extract control parameters (excludeResponseParameter)
                    control_params = {
                        k: v
        ...
        for descriptor in descriptors:
            ...
            if "properties" in descriptor:
                for prop_name, prop_info in descriptor["properties"].items():
                    ...
                    tool_desc = {
                        ...
                        "function": {
                            "name": tool_name,
                            "description": f"Query{device_desc}of{prop_info['description']}",
                            "parameters": {
                                "type": "object",
                                "properties": {
                                    "response_success": {
                                        "type": "string",
                                        "description": f"Friendly reply on successful query, must use{{value}}Use as placeholder for queried value",
                                    },
                                    "response_failure": {
                                        "type": "string",
            ...
            if "methods" in descriptor:
                for method_name, method_info in descriptor["methods"].items():
                    ...
                    # Add original parameters of method
                    ...
                    parameters.update(
                        {
                            "response_success": {
                                "type": "string",
                                "description": "Operation succeededfriendly reply when",
                            },
                            "response_failure": {
                                "type": "string",
                                "description": "Operation failedfriendly reply when",
                            },
                        }
                    )
    ...
    def has_tool(self, tool_name: str) -> bool:
        """Check whether specified device-side IoT tool exists"""
