...
class ServerMCPManager:
    """Centralized manager for multiple server MCP services"""
    ...
    def __init__(self, conn) -> None:
        ...
        if not os.path.exists(self.config_path):
            ...
            logger.bind(tag=TAG).warning(
                f"Check mcp service config file: data/.mcp_server_settings.json"
            )
    ...
    async def initialize_servers(self) -> None:
        ...
        # Output currently supported serversMCPTool list
        if hasattr(self.conn, "func_handler") and self.conn.func_handler:
            # Refresh tool cacheto ensure serverMCPTool loaded correctly
    ...
    async def execute_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Any:
        """Execute tool call, try reconnect on failure"""
        logger.bind(tag=TAG).info(f"Execute server MCP tool {tool_name}, parameters: {arguments}")

        max_retries = 3  # Max retry count
        retry_interval = 2  # Retry Interval(seconds)

        # Found corresponding client
        client_name = None
        target_client = None
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Throw directly on last attempt failureException
                ...
                logger.bind(tag=TAG).info(
                    f"Try reconnecting MCP client {client_name} before retry"
                )
                try:
