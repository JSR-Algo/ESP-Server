...
class ServerMCPClient:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        Args:
            config: MCP service config dict
        """
    ...
    async def call_tool(self, name: str, arguments: dict, read_timeout_seconds: timedelta | None = None, progress_callback: ProgressFnT | None = None, *, meta: dict[str, Any] | None = None) -> Any:
        ...
        Raises:
            RuntimeError: Throw when client not initialized
        """
        if not self.session:
    ...
    def is_connected(self) -> bool:
        """Check whether MCP client is connected normally

        Returns:
            bool: Return True if client is connected and working normally, otherwise False
        """
        # Check whether work task exists
        if self._worker_task is None:
            return False

        # Check whether work task completed or canceled
        ...
        # All checks passed, connection normal
        return True
            ...
            try:
                ...
                # EstablishSSEClient
                    ...
                    if "API_ACCESS_TOKEN" in self.config:
                        ...
                        self.logger.bind(tag=TAG).warning(f"YouUsing old outdated config API_ACCESS_TOKEN Please in.mcp_server_settings.jsonIn willAPI_ACCESS_TOKENSet directly onheadersin, for example 'Authorization': 'Bearer API_ACCESS_TOKEN'")
                   
                    # Select different client based on transport type, default SSE
                    transport_type = self.config.get("transport", "sse")

                    if transport_type == "streamable-http" or transport_type == "http":
                        # Use Streamable HTTP Transfer
                    ...
                    raise ValueError("MCP clientConfig must contain'command'or'url'")
                ...
                # Suspend waiting close
                await self._shutdown_evt.wait()

            except Exception as e:
                self.logger.bind(tag=TAG).error(f"Server sideMCP client worker coroutineError: {e}")
                self._ready_evt.set()
                raise
