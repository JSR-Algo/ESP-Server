...
class MCPEndpointExecutor(ToolExecutor):
    ...
    async def execute(
        ...
        try:
            # Convert parameter toJSONString
            import json

            args_str = json.dumps(arguments) if arguments else "{}"

            # CallMCPAccess point tool
            ...
            # Vision large model skips secondaryLLMProcess
            if (
    ...
    def has_tool(self, tool_name: str) -> bool:
        """Check whether specified MCP access point tool exists"""
        if (
            not hasattr(self.conn, "mcp_endpoint_client")
            or not self.conn.mcp_endpoint_client
        ):
