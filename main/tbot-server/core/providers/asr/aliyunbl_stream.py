...
class ASRProvider(ASRProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        self.task_id = None  # Current TaskID
    ...
    async def receive_audio(self, conn, audio, audio_have_voice):
        ...
        # Only when hasVoiceEstablish connection when none exists
        ...
        if self.asr_ws and self.is_processing and self.server_ready:
            try:
                ...
                # Send DirectlyPCMAudio Data(Binary)
    ...
    async def _start_recognition(self, conn: "ConnectionHandler"):
        ...
        try:
            # If manual mode,Set timeout duration to max value
    ...
    def _build_run_task_message(self) -> dict:
        ...
        # Only whenModel nametov2Add only at endvocabulary_idParameter
            ...
            while not conn.stop_event.is_set():
                ...
                try:
                    ...
                    # Processresult-generatedEvent
                        ...
                        # Determine if final result(sentence_endforTrueandend_timeNot fornull)
                        ...
                        if is_final:
                            ...
                            if conn.client_listen_mode == "manual":
                                ...
                                # In manual mode,Only after receivedstopTrigger processing only after signal
                    ...
                    # Processtask-finishedEvent
                    elif event == "task-finished":
                        logger.bind(tag=TAG).debug("Task completed")
                        break

                    # Processtask-failedEvent
    ...
    async def _send_stop_request(self):
        ...
        if self.asr_ws:
            try:
                ...
                logger.bind(tag=TAG).error(f"Send stop request failed: {e}")
    ...
    async def _cleanup(self):
        ...
        if self.asr_ws:
            try:
                # firstSend finish-task command
                await self._send_finish_task()
                # Wait short time for server processing
