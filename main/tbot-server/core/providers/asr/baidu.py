    ...
    async def speech_to_text(
        ...
        try:
            ...
            if not self.app_id or not self.api_key or not self.secret_key:
                logger.bind(tag=TAG).error("Baidu speech recognition config not set, cannot recognize")
                return None, None
            ...
            if result and result["err_no"] == 0:
                ...
                raise Exception(
                    f"Baidu speech recognition failed,Errorcode: {result['err_no']},Error info: {result['err_msg']}"
                )
