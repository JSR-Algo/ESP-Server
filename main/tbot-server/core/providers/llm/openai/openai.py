...
# NeedDisablePlatform domains for thinking mode and corresponding parameters (thinking mode off by default)
...
class LLMProvider(LLMProviderBase):
    def __init__(self, config):
        ...
        if isinstance(timeout_config, dict):
            ...
            # Compatible with old single timeout config (integer or float)
            custom_timeout = httpx.Timeout(timeout_config)
        else:
            # Not configuredor config invalid, use default value
    ...
    def normalize_dialogue(dialogue):
        """Auto-fix messages missing content in dialogue"""
        for msg in dialogue:
    ...
    def response(self, session_id, dialogue, **kwargs):
        ...
        # Add optional parameters,Only when param notNoneAdd Only When
        optional_params = {
            ...
            for chunk in stream:
                if getattr(chunk, "choices", None):
                    ...
                    logger.bind(tag=TAG).info(
                        f"Token Cost: input {getattr(usage_info, 'prompt_tokens', 'Unknown')},"
