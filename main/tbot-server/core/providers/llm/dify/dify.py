...
class LLMProvider(LLMProviderBase):
    def __init__(self, config):
        ...
        self.session_conversation_map = {}  # Storesession_idandconversation_idMapping of
        model_key_msg = check_model_key("DifyLLM", self.api_key)
        if model_key_msg:
            logger.bind(tag=TAG).error(model_key_msg)

    def response(self, session_id, dialogue, **kwargs):
        # Get last userMessage
            ...
            if self.mode == "chat-messages":
                for line in r.iter_lines():
                    if line.startswith(b"data: "):
                        ...
                        # If not foundconversation_id,then get thisconversation_id
                        if not conversation_id:
                            conversation_id = event.get("conversation_id")
                            self.session_conversation_map[session_id] = (
                                conversation_id  # Update Mapping
                            )
                        # Filter message_replace Event, thisEventwill push full once
    ...
    def response_with_functions(self, session_id, dialogue, functions=None):
        if len(dialogue) == 2 and functions is not None and len(functions) > 0:
            # First callllmTake last userMessage, appendtoolPromptword
        ...
        # If last one is role="tool", append touseron
