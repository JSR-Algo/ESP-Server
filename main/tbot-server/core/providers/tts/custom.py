...
class TTSProvider(TTSProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        if isinstance(self.params, str):
            try:
                ...
                raise ValueError("Custom TTS config parameter error, cannot parse string as object")
        elif not isinstance(self.params, dict):
            raise TypeError("Custom TTS config parameter error, see config instructions")
    ...
    async def text_to_speak(self, text, output_file):
        ...
        if resp.status_code == 200:
            ...
            error_msg = f"Custom TTSRequest failed: {resp.status_code} - {resp.text}"
            logger.bind(tag=TAG).error(error_msg)
            raise Exception(error_msg)  # ThrowExceptionLet caller catch
