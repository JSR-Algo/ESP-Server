...
class TTSProvider(TTSProviderBase):
    ...
    async def open_audio_channels(self, conn):
        """Initialize audio channel and update config based on conn.sample_rate"""
        # Call parent method
        await super().open_audio_channels(conn)

        # Updateaudio_settingSample rate in is actualconn.sample_rate
    ...
    async def text_to_speak(self, text, is_last):
        """Stream TTS audio; push audio list once per sentence"""
        payload = {
            "model": self.model,
            "text": text,
            "stream": True,
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if resp.status != 200:
                        logger.bind(tag=TAG).error(
                            f"TTSRequest failed: {resp.status}, {await resp.text()}"
                        )
                    ...
                    async for chunk in resp.content.iter_any():
                        ...
                        while True:
                            ...
                            # Extract single completeJSONblock
                            ...
                            try:
                                ...
                                # Only processstatus=1Valid audio block ignorestatus=2end summary block
                    ...
                    # flush Data less than one frame remains
                    if self.pcm_buffer:
                        self.opus_encoder.encode_pcm_to_opus_stream(
                    ...
                    # If last segment, output audio retrieval complete
                    if is_last:
                        self._process_before_stop_play_files()
    ...
    def to_tts(self, text: str) -> list:
        ...
        Returns:
            list: returns list of opus-encoded audio data
        """
        ...
        try:
            with requests.post(
                ...
                if response.status_code != 200:
                    logger.bind(tag=TAG).error(
                        f"TTSRequest failed: {response.status_code}, {response.text}"
                    )
                    return []

                logger.info(f"TTS request succeeded: {text}, time: {time.time() - start_time}s")

                # UseopusEncoder processingPCMData
                opus_datas = []
