...
class TTSProvider(TTSProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        # CreateOpusEncoder note interface returned sample rate is24000
    ...
    def audio_to_pcm_data_stream(
        ...
        """Convert audio file to PCM encoding, use 24kHz sampling rate"""
    ...
    def to_tts(self, text: str) -> list:
        ...
        try:
            with requests.post(self.api_url, json=payload, timeout=5) as response:
                ...
                # Frame ProcessingPCMData
                for i in range(0, len(pcm_data), frame_bytes):
                    frame = pcm_data[i : i + frame_bytes]
                    if len(frame) < frame_bytes:
                        # Last frame may be insufficient, use0Fill
