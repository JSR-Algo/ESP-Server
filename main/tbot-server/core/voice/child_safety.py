...
_MODEL_OUTPUT_BLOCK_PATTERNS = (
    ...
    ("self_harm", re.compile(r"\b((?:kill|hurt|harm|injure|poison|starve|cut|hang)\s+(?:yourself|myself)|cut\s+(?:your|my)\s+wrist|overdose|jump\s+off\s+(?:a\s+)?(?:bridge|building|roof|window)|end\s+(?:your|my)\s+life|take\s+(?:your|my)\s+own\s+life|unalive\s+(?:yourself|myself)|suicide|self\s*harm|tu\s+tu|tự\s+tử)\b", re.I)),
