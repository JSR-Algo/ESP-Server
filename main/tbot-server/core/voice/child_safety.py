...
_MODEL_OUTPUT_BLOCK_PATTERNS = (
    ...
    ("pii_child_name", re.compile(r"\b((?:tell|give|share|say)\s+(?:me\s+)?your\s+name|what(?:'s|\s+is)\s+your\s+name|con\s+ten\s+gi|ten\s+con\s+la\s+gi|tên\s+con\s+là\s+gì|cho\s+(?:minh|mình)\s+biết\s+tên\s+con)\b", re.I)),
    ("pii_school_query", re.compile(r"\b(which\s+school\s+do\s+you\s+go\s+to|what\s+school\s+do\s+you\s+go\s+to|con\s+hoc\s+truong\s+nao|con\s+học\s+trường\s+nào)\b", re.I)),
    ("pii_location_query", re.compile(r"\b(where\s+do\s+you\s+live|where\s+are\s+you\s+from|con\s+song\s+o\s+dau|con\s+sống\s+ở\s+đâu)\b", re.I)),
    ("pii_password", re.compile(r"\b((?:tell|give|share|send)\s+(?:me\s+)?(?:your\s+)?password|what(?:'s|\s+is)\s+your\s+password|mat\s+khau|mật\s+khẩu)\b", re.I)),
    ("pii_photo", re.compile(r"\b((?:send|share|give)\s+(?:me\s+)?(?:your\s+)?(?:photo|picture|selfie)|(?:gui|gửi)\s+(?:anh|ảnh)\s+(?:cua|của)\s+con)\b", re.I)),
    ...
    ("move_to_private_app", re.compile(r"\b((?:message|dm|chat|text|add)\s+(?:me\s+)?(?:on|via)\s+(?:zalo|telegram|whatsapp|messenger|facebook|instagram|tiktok|discord)|(?:move|switch|go)\s+to\s+(?:zalo|telegram|whatsapp|messenger|facebook|instagram|tiktok|discord)|chuyen\s+sang\s+(?:zalo|telegram|whatsapp|messenger|facebook|instagram|tiktok|discord)|chuyển\s+sang\s+(?:zalo|telegram|whatsapp|messenger|facebook|instagram|tiktok|discord)|nhan\s+tin\s+rieng|nhắn\s+tin\s+riêng)\b", re.I)),
    ...
    ("self_harm", re.compile(r"\b((?:kill|hurt|harm|injure|poison|starve|cut|hang|choke|drown|burn|stab|shoot)\s+(?:yourself|myself)|cut\s+(?:your|my)\s+wrist|overdose|drink\s+bleach|(?:take|swallow)\s+(?:all\s+)?(?:(?:your|my)\s+)?pills|jump\s+off\s+(?:a\s+)?(?:bridge|building|roof|window)|end\s+(?:your|my)\s+life|take\s+(?:your|my)\s+own\s+life|unalive\s+(?:yourself|myself)|suicide|self\s*harm|tu\s+tu|tự\s+tử)\b", re.I)),
