...
class GlobalCacheManager:
    ...
    def set(
        ...
        # Use configuredTTLor Passed InTTL
        ...
        with self._locks[cache_name]:
            ...
            if config.strategy in [CacheStrategy.LRU, CacheStrategy.TTL_LRU]:
                # LRUStrategy: move to end if already exists
                if key in cache:
                    del cache[key]
                cache[key] = entry
        ...
        # PeriodicClean up expired entries
    ...
    def _maybe_cleanup(self, cache_name: str):
        ...
        if now - self._last_cleanup > config.cleanup_interval:
            ...
            if deleted > 0:
                ...
                self.logger.debug(f"Clean cache {cache_name}: deleted {deleted} expired entries")


# CreateGlobal cache managerInstance
