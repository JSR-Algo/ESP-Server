        ...
        while True:
            try:
                ...
                if time.time() - self.start_time > self.timeout:
                    raise TimeoutError("Get file lock timed out")
                time.sleep(0.1)
...
class WakeupWordsConfig:
    ...
    def _load_config(self) -> Dict:
        ...
        # If cache valid, return cache directly
        if (
    ...
    def _save_config(self, config: Dict):
        ...
        try:
            ...
            print(f"Save config file failed: {e}")
            raise
        except Exception as e:
    ...
    def update_wakeup_response(self, voice: str, file_path: str, text: str):
        ...
        try:
            ...
            print(f"Update wake word reply config failed: {e}")
            raise

    def generate_file_path(self, voice: str) -> str:
        """Generate audio file path, use voice hash as filename"""
        try:
            # Generatevoicehash value
            voice_hash = hashlib.md5(voice.encode()).hexdigest()
            file_path = os.path.join(self.assets_dir, f"{voice_hash}.wav")

            # If file exists, firstDelete
            ...
            print(f"Generate audio file path failed: {e}")
            raise