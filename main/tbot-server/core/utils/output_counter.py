...
# Global dict, store daily output word count for each device
...
def get_device_output(device_id: str) -> int:
    """
    Get device output word count for current day
    """
...
def add_device_output(device_id: str, char_count: int):
    """
    Increase device output word count
    """
    current_date = datetime.datetime.now().date()
...
def check_device_output_limit(device_id: str, max_output_size: int) -> bool:
    """
    Check whether device exceeds output limit
    :return: True if over limit, False if not
    """
    if not device_id:
        return False
