...
1. **Google Live hiện KHÔNG gửi `safety_settings`; đây là quyết định tương thích SDK, không phải thiếu sót vô tình.** `core/voice/google_live/client.py:_build_connect_config()` chỉ dựng transcription, speech, realtime input, session resumption, context-window compression, tools, và `system_instruction`; test `test_build_connect_config_omits_sdk_forbidden_safety_settings` khóa lại rằng `types.LiveConnectConfig(**config)` hợp lệ khi **không** có `safety_settings`. SDK `google-genai` hiện từ chối field đó trên Live config (`extra_forbidden`). Lớp an toàn đang có là: prompt-level `ensure_child_safety_block()`, bộ lọc output transcript `screen_model_output()` / `_block_unsafe_model_output()` trước khi audio unsafe tới websocket, và lesson child-response window ép `session_mode=LESSON` trước khi forward audio (`session_provider/google_live.py:381,931,1048`) để audio bridge drop mọi model output không phải lesson prompt (`audio_bridge.py:529-550`). Đây là đường transcription-only deterministic cho câu trả lời của trẻ, không phải strict Live safety settings.
...
5. **Phiên Live mở.** `_resolve_functions_for_live()` (`google_live.py:848–860`) dựng tool list từ `product_tool_names(conn)`. **Vì lesson runtime đã bật qua auto-enable hoặc `LESSON_RUNTIME_ENABLED=true`, `product_toolset.py:65–66` đẩy `start_lesson` vào danh sách → tool có mặt trong `connect_config.tools`.** `_build_connect_config` (`client.py`) đặt `system_instruction` = persona phần A đã bọc child-safety và `tools` = `[start_lesson, …]`. `safety_settings` không được gắn vào Live config vì SDK từ chối field này; output transcript moderation là lớp kiểm soát runtime đang có. Trẻ trò chuyện bình thường.
...
- `core/voice/google_live/client.py` (`_build_connect_config` intentionally omits SDK-forbidden `safety_settings`; `test_build_connect_config_omits_sdk_forbidden_safety_settings` validates the Live config); `_build_system_instruction` đọc `prompt`/`system_prompt`, bọc `ensure_child_safety_block`, gắn làm `system_instruction`; `_build_tools`/`_sanitize_schema`; `_normalize_tool_call`.
