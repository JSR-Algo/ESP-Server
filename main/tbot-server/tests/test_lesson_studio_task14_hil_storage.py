...
def test_repo_native_fault_driver_dependency_self_tests_offline(monkeypatch, capsys):
    fault = load_script("lesson_studio_task14_fault_driver.py")
    monkeypatch.setattr(
        sys, "argv", ["lesson_studio_task14_fault_driver.py", "--self-test"]
    )
...
@pytest.mark.parametrize(
    "location",
    [
        "http://127.0.0.1:8003/redirected",
        "http://other.invalid/redirected",
    ],
)
def test_raw_mcp_transport_never_redirects_mint_secret(
    monkeypatch, status, location
):
    hil = load_script("lesson_studio_task14_hil_storage.py")
    secret = "mint-secret-must-not-redirect"
    requests = []
    handlers = []
    real_build_opener = hil.urllib.request.build_opener

    class RedirectingHttpHandler(hil.urllib.request.HTTPHandler):
    ...
    def build_opener(*values):
        ...
        return real_build_opener(*values, RedirectingHttpHandler())

    monkeypatch.setattr(hil.urllib.request, "build_opener", build_opener)
    monkeypatch.setattr(
        hil.urllib.request,
        "urlopen",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("global redirect-following urlopen must not be used")
        ),
    )

    transport = hil.RawMcpTransport(
        "http://127.0.0.1:8003", "device-uuid", secret
    )
    with pytest.raises(hil.HilTransportError, match="HIL_HTTP_ERROR") as caught:
        transport.call("self.lesson_assets.hil.status", {}, 5)

    assert len(requests) == 1
    assert requests[0].full_url.endswith("/mcp-call")
    assert requests[0].get_header("X-mint-secret") == secret
    assert secret not in str(caught.value)
    redirect_handler = next(
        value for value in handlers if isinstance(value, hil.NoRedirectHandler)
    )
    assert (
        redirect_handler.redirect_request(
            requests[0], None, status, "redirect", {}, location
        )
        is None
    )
...
def test_build_manifest_hash_and_parse_share_one_read_when_path_swaps(
    tmp_path, monkeypatch
):
    identity = load_script("lesson_studio_task14_build_identity.py")
    manifest_path, manifest, _paths = task6_manifest(tmp_path)
    original_source = manifest["sourceCommit"]
    replacement = {**manifest, "sourceCommit": "f" * 40}
    real_read = identity._read_manifest_bytes

    def read_then_swap(path):
        manifest_bytes = real_read(path)
        manifest_path.write_text(json.dumps(replacement, sort_keys=True) + "\n")
        return manifest_bytes

    monkeypatch.setattr(identity, "_read_manifest_bytes", read_then_swap)

    result = identity.load_build_identity(manifest_path, expected_profile="hil")

    assert result["sourceCommit"] == original_source
    assert json.loads(manifest_path.read_text())["sourceCommit"] == "f" * 40
