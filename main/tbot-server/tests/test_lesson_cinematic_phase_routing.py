...
@pytest.mark.asyncio
async def test_v5_google_live_receive_timeout_after_prompt_resend_enters_response_policy() -> None:
    runtime = _v5_runtime()
    runtime.state = S_RUNNING
    ...
    assert runtime._safe_speaking().attempts == 2
    assert runtime.state == S_RUNNING
...
@pytest.mark.asyncio
async def test_v5_phase_less_later_step_forwards_ordered_step_started_once() -> None:
    runtime = _v5_runtime()
    runtime.state = S_RUNNING
    runtime._step_index = 0
    runtime._step = runtime._steps[0]
