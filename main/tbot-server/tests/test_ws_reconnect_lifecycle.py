...
async def test_supersede_tolerates_a_handler_without_a_socket(monkeypatch):
    server = _build_server(monkeypatch)
    displaced = types.SimpleNamespace(websocket=None)
    server._supersede_connection(DEVICE_ID, displaced)
    await asyncio.sleep(0)
    await asyncio.sleep(0)
...
async def test_supersede_logs_and_survives_a_close_failure(monkeypatch):
    ...
    server._supersede_connection(DEVICE_ID, types.SimpleNamespace(websocket=_Boom()))
    for _ in range(10):
