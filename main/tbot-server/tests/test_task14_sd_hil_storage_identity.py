...
def test_arm_and_actual_triggers_carry_the_latest_inspected_binding():
    module = load_driver()
    remote_build = {
        "schemaVersion": 1,
        "hilProfile": "task14-hil-v1",
    ...
    class Transport:
        ...
        def call(self, tool, args, _timeout):
            self.calls.append((tool, args))
            if tool == module.HIL_TOOL_NAMES["arm"]:
                return {
                    "cacheKey": cache_key,
                    "status": "armed",
                    "operation": "evict",
            ...
            if tool == module.TRIGGER_TOOLS["evict"]:
                return {
                    "cacheKey": cache_key,
                    "status": "not_found",
                    "evicted": False,
                ...
                "files": [
                    {
                        "key": "hil-asset.png",
                        "path": "hil-asset.png",
                        "localPath": "/sdcard/hil-asset.png",
                        "state": "DOWNLOADED",
                        "bytes": 1,
                    }
                ],
    ...
    client = module.HilToolClient(transport, local_build)
    client.build_identity = remote_build
    client.build_identity_id = identity_id
    client.connection_binding_id = "connection-one"
    arguments = SimpleNamespace(
        asset_sha256="d" * 64,
        asset_url="http://127.0.0.1/asset",
        asset_bytes=1,
    )

    client.arm(cache_key, "evict", "before_first_unlink", "fail")
    module._trigger(client, "evict", cache_key, arguments)
    module._trigger(client, "sync", cache_key, arguments)

    for _tool, args in transport.calls:
        assert args["expectedBuildIdentityId"] == identity_id
        assert args["expectedConnectionBindingId"] == "connection-one"
