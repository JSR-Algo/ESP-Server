...
def analyze(text, count=100, gate=20*1024, tolerance=64*1024):
    ...
    loss_above_tolerance=len(ps)>=3 and ps[0]-ps[-1]>tolerance
    ordered=bool(steps) and all(current>previous for previous,current in zip(steps,steps[1:]))
    checks={'at_least_100_transitions':len(steps)>=count,'transition_sequence_strictly_increasing':ordered,'psram_samples_present':len(ps)>=3,'no_monotonic_psram_loss':not leak,'no_psram_loss_above_tolerance':not loss_above_tolerance,'internal_sram_above_gate':bool(sr) and min(sr)>=gate,'no_firmware_reset':not RESET.search(text)}
    return {'status':'PASS' if all(checks.values()) else 'NOT_PASS','checks':checks,'metrics':{'transitions':len(steps),'psramFirst':ps[0] if ps else None,'psramLast':ps[-1] if ps else None,'psramNetLoss':ps[0]-ps[-1] if ps else None,'internalSramMin':min(sr) if sr else None}}
...
def main():
    ...
    if a.self_test:
        ...
        assert analyze(stable)['status']=='PASS'
        leaking='\n'.join(f'lesson_step_transition sequence={i} psram_free={8000000-i*2048+(4096 if i==50 else 0)} internal_min_free=32768' for i in range(100))
        assert analyze(leaking)['checks']['no_psram_loss_above_tolerance'] is False
