...
| --- | --- | --- |
| Deterministic full sessions | `python3 scripts/course_mode_task06_runtime_validation.py --backend-root ... --firmware-root ... --iterations 2000 --min-duration-seconds 60 --output .../metrics/runtime-soak-60s.json` | PASS: 22 scenario types, 159,927 iterations, 3,518,394 sessions, 60.000015 s, zero mismatches |
...
| Reliability/resource trend | 60-second bounded wall-clock soak with tracemalloc, FD, thread, and process metrics | PASS: thread delta 0, FD delta 0, heap current delta 208 B against a 1 MiB fail-closed bound, heap peak 4,880 B, max RSS 33,112,064 B |
...
Three independent read-only reviews covered the complete scoped diffs. Backend and firmware reviews found no actionable issue and confirmed that the fixture changes preserve the original assertions. Initial ESP review found that SHA identity was reported but not enforced, the validation test depended on disposable worktree paths, and ignored evidence would not be committed. Built-in commit review then found that the evidence commit could not rerun its own harness and that dirty runtime paths were not rejected. Parent review additionally found that executable validation files could remain dirty while passing and that retained heap growth was measured but not gated. All findings were fixed: executable validation changes must be committed, only evidence artifacts may remain dirty during regeneration, unexpected runtime changes fail the gate, retained heap growth has a 1 MiB fail-closed bound, the opt-in cross-repository tests passed 5 tests, and the final 60-second soak passed with all three runtime trees matching their frozen bases. Security-pattern scans and `git diff --check` were clean.
