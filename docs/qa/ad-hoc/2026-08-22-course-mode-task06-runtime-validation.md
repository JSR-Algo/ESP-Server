...
| --- | --- | --- |
| Deterministic full sessions | `python3 scripts/course_mode_task06_runtime_validation.py --backend-root ... --firmware-root ... --iterations 2000 --min-duration-seconds 60 --output .../metrics/runtime-soak-60s.json` | PASS: 22 scenario types, 160,114 iterations, 3,522,508 sessions, 60.000166 s, zero mismatches |
...
| Reliability/resource trend | 60-second bounded wall-clock soak with tracemalloc, FD, thread, and process metrics | PASS: thread delta 0, FD delta 0, heap current delta 208 B, heap peak 4,880 B, max RSS 32,374,784 B |
...
Three independent read-only reviews covered the complete scoped diffs. Backend and firmware reviews found no actionable issue and confirmed that the fixture changes preserve the original assertions. Initial ESP review found that SHA identity was reported but not enforced, the validation test depended on disposable worktree paths, and ignored evidence would not be committed. All three findings were fixed; the SHA gate and opt-in cross-repository test passed 3 tests, a fresh 60-second soak passed, and final ESP re-review returned no remaining actionable finding. Security-pattern scans and `git diff --check` were clean.
